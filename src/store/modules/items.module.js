...
export const items = {
    ...
    actions: {
        ...
        get({commit, dispatch}, payload) {
            ...
            return ItemsService.getItemData(payload).then(({success, response, errors}) => {
                if (success) {
        ...
        list({commit, dispatch, state}, params) {
            ...
            return ItemsService.getItems(params).then(({success, items, paginator, errors}) => {
                if (success) {
        ...
        update({commit, dispatch}, payload) {
            ...
            return ItemsService.updateItem(payload).then(({success, errors}) => {
                if (success) {
                    dispatch("alerts/add_success", "Item updated successfully", {root: true});
                    commit("setLoading", false);
                    routes.back();
                } else {
                    commit("setLoading", false);
        ...
        create({commit, dispatch}, payload) {
            ...
            return ItemsService.create(payload).then(({success, errors}) => {
                if (success) {
                    dispatch("alerts/add_success", "Item created successfully", {root: true});
                    commit("setLoading", false);
                    routes.back();
                } else {
                    commit("setLoading", false);
        ...
        destroy({dispatch}, payload) {
            return ItemsService.destroy(payload).then(({success, errors}) => {
                if (success) {
                    dispatch("alerts/add_success", "Item deleted successfully", {root: true});
                    routes.push('/Items');
                } else {
                    dispatch("alerts/add_error", errors.message, {root: true});
                }
        ...
        downloadQr({commit, dispatch}, payload) {
            commit("setLoading", true);
            return ItemsService.getQrDownload(payload).then(({success, qr, errors}) => {
                if (success) {
    ...
    mutations: {
        ...
        getItemSuccessfully(state, response) {
            ...
            state.selectedItem = new Item(response.id, response.name, response.location, response.quantity, response.description, category)
        ...
        getItemsSuccessfully(state, response) {
            ...
            temp = response.map((res) => {
                ...
                return new Item(res.id, res.name, res.location, res.quantity, res.description, category)
            })
            console.log(temp)
            state.items = temp
        },
