...
export const users = {
    ...
    actions: {
        ...
        create({commit, dispatch}, payload) {
            ...
            return UsersService.create(payload).then(({success, errors}) => {
                if (success) {
                    dispatch("alerts/add_success", "User created successfully", {root: true});
                    commit("setLoading", false);
                    routes.back();
                } else {
                    commit("setLoading", false);
        ...
        destroy({dispatch}, payload) {
            return UsersService.destroy(payload).then(({success, errors}) => {
                if (success) {
                    dispatch("alerts/add_success", "User deleted successfully", {root: true});
                    routes.push('/users');
                } else {
                    dispatch("alerts/add_error", errors.message, {root: true});
                }
    ...
    mutations: {
        ...
        getUserSuccessfully(state, response) {
            ...
            state.selectedUser = new User(response.id, response.name, response.surname, response.email, additionalInfo, roles)
        ...
        getUsersSuccessfully(state, response) {
            ...
            temp = response.map((res) => {
                let additionalInfo = new Additionalinfo(res.more_info?.phone_number, res.more_info?.address, res.more_info?.about_me);
                let roles = response.roles?.map(role => {
                    return new Roles(role?.id, role?.name)
                });
                return new User(res.id, res.name, res.surname, res.email, additionalInfo, roles)
            })
            state.users = temp
        },
