...
export const categories = {
    ...
    actions: {
        ...
        list({commit, dispatch, state}, params) {
            ...
            return CategoriesService.index(params).then(({success, response, paginator}) => {
                if (success) {
        ...
        set({commit, dispatch}, payload) {
            ...
            return CategoriesService.create(params).then(({success, response}) => {
                if (success) {
                    dispatch("alerts/add_success", "Category created successfully", {root: true});
                    commit("SET_LOADING", false);
                    routes.back();
                } else {
                    commit("SET_LOADING", false);
        ...
        update({commit, dispatch}, payload) {
            ...
            return CategoriesService.update(payload).then(({success, errors}) => {
                if (success) {
                    dispatch("alerts/add_success", "Category updated successfully", {root: true});
                    commit("SET_LOADING", false);
                    routes.back();
                } else {
                    commit("SET_LOADING", false);
        ...
        destroy({commit, dispatch}, payload) {
            ...
            return CategoriesService.delete(payload).then(({success, errors}) => {
                if (success) {
                    dispatch("alerts/add_success", "Category deleted successfully", {root: true});
                    commit("SET_LOADING", false);
                    routes.back();
                } else {
                    commit("SET_LOADING", false);
    ...
    mutations: {
        ...
        SET_CATEGORY(state, category) {
            state.selectedCategory = new Category(category.data.id, category.data.name);
        },
