...
export const roles = {
  ...
  actions: {
    ...
    list({ commit, dispatch, state }, params) {
      ...
      return RolesService.index(params).then(
    ...
    set({ commit, dispatch }, payload) {
      ...
      return RolesService.create(params).then(({ success, response }) => {
        if (success) {
          dispatch("alerts/add_success", "Role created successfully", {
            root: true,
          });
          commit("SET_LOADING", false);
          routes.back();
    ...
    update({ commit, dispatch }, payload) {
      ...
      return RolesService.update(payload).then(({ success, errors }) => {
        if (success) {
          dispatch("alerts/add_success", "Role updated successfully", {
            root: true,
          });
          commit("SET_LOADING", false);
          routes.back();
    ...
    destroy({ commit, dispatch }, payload) {
      ...
      return RolesService.delete(payload).then(({ success, errors }) => {
        if (success) {
          dispatch("alerts/add_success", "Role deleted successfully", {
            root: true,
          });
          commit("SET_LOADING", false);
          routes.back();
    ...
    getPermissions({ commit, dispatch }, payload) {
      commit("SET_LOADING", true);
      return RolesService.getPermissions(payload).then(
        ({ success, response }) => {
          if (success) {
    ...
    setPermissions({ commit, dispatch }, payload) {
      commit("SET_LOADING", true);
      return RolesService.setPermissions(payload.id, payload.permissions).then(
        ({ success, response }) => {
          if (success) {
            commit("SET_LOADING", false);
            dispatch("alerts/add_success", "Permissions has been updated", {
              root: true,
            });
            routes.back();
          } else {
