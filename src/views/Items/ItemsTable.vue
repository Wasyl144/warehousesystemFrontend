            ...
            <template v-slot:header>
              <div class="row">
                ...
                <div class="col-2 text-right">
                  <base-button type="primary" v-if="can('items.store')" size="sm" @click="addItem()" icon="fa fa-plus">Add Item</base-button>
                </div>
              </div>
            </template>
                    ...
                    <template v-slot:default="row">
                      ...
                      <td class="text-right">
                        <base-dropdown class="dropdown" position="right">
                          ...
                          <template v-slot:default>
                            <router-link v-if="can('items.show')" class="dropdown-item"
                                         :to="{ name: 'showItems', params: { id: row.item.id }}">Show item
                            </router-link>
                            <router-link v-if="can('items.update')" class="dropdown-item"
                                         :to="{ name: 'editItems', params: { id: row.item.id }}">Edit
                            </router-link>
                            <a class="dropdown-item" v-if="can('items.destroy')" @click="destroy(row.item)" href="#">Delete</a>
                          </template>
                        </base-dropdown>
                      </td>
                    </template>
