            ...
            <template v-slot:header>
              <div class="row">
                ...
                <div class="col-2 text-right">
                  <base-button type="primary" v-if="can('user.store')" size="sm" @click="addUser()" icon="fa fa-plus">Add user</base-button>
                </div>
              </div>
            </template>
                    ...
                    <template v-slot:default="row">
                      ...
                      <td class="text-right">
                        <base-dropdown class="dropdown" position="right">
                          ...
                          <template v-slot:default>
                            <router-link v-if="can('user.show')" class="dropdown-item"
                                         :to="{ name: 'showUser', params: { id: row.item.id }}">Show profile
                            </router-link>
                            <router-link v-if="can('user.update')" class="dropdown-item"
                                         :to="{ name: 'editUser', params: { id: row.item.id }}">Edit
                            </router-link>
                            <a class="dropdown-item" v-if="can('user.destroy')" @click="destroy(row.item)" href="#">Delete</a>
                          </template>
                        </base-dropdown>
                      </td>
                    </template>
